...
func NewController(opts Opts, stopChan <-chan struct{}) (*Controller, error) {
	...
	for _, nodeGroupOpts := range opts.NodeGroups {
		...
		if nodeGroupOpts.autoDiscoverMinMaxNodeOptions() {
			...
			opts.Logger.Debugf("auto discovered min_nodes = %v for node group %v", nodeGroupOpts.MinNodes, nodeGroupOpts.Name)
			nodeGroupOpts.MaxNodes = int(cloudProviderNodeGroup.MaxSize())
			opts.Logger.Debugf("auto discovered max_nodes = %v for node group %v", nodeGroupOpts.MaxNodes, nodeGroupOpts.Name)
		}
...
func (c *Controller) scaleNodeGroup(nodegroup string, nodeGroup *NodeGroupState) (int, error) {
	...
	if err != nil {
		c.logger.Errorf("Failed to list pods: %v", err)
		return 0, err
	}
	...
	// Metrics and Logs
	c.logger.WithField("nodegroup", nodegroup).Infof("pods total: %v", len(pods))
	c.logger.WithField("nodegroup", nodegroup).Infof("nodes remaining total: %v", len(allNodes))
	c.logger.WithField("nodegroup", nodegroup).Infof("cordoned nodes remaining total: %v", len(cordonedNodes))
	c.logger.WithField("nodegroup", nodegroup).Infof("nodes remaining untainted: %v", len(untaintedNodes))
	c.logger.WithField("nodegroup", nodegroup).Infof("nodes remaining tainted: %v", len(taintedNodes))
	...
	if len(allNodes) == 0 {
		...
		c.logger.WithField("nodegroup", nodegroup).Warning(err.Error())
		return 0, err
	}
	if len(allNodes) < nodeGroup.Opts.MinNodes {
		err = errors.New("node count less than the minimum")
		c.logger.WithField("nodegroup", nodegroup).Warningf(
	...
	if err != nil {
		c.logger.Errorf("Failed to calculate requests: %v", err)
		return 0, err
	}
	memCapacity, cpuCapacity, err := k8s.CalculateNodesCapacityTotal(untaintedNodes)
	if err != nil {
		c.logger.Errorf("Failed to calculate capacity: %v", err)
		return 0, err
	}
	...
	if len(untaintedNodes) < nodeGroup.Opts.MinNodes {
		c.logger.WithField("nodegroup", nodegroup).Warn("There are less untainted nodes than the minimum")
		...
		if err != nil {
			c.logger.WithField("nodegroup", nodegroup).Error(err)
		}
		return result, err
	}
	...
	if err != nil {
		c.logger.Errorf("Failed to calculate percentages: %v", err)
		return 0, err
	}

	// Metrics
	c.logger.WithField("nodegroup", nodegroup).Infof("cpu: %v, memory: %v", cpuPercent, memPercent)
	...
	if locked {
		...
		c.logger.WithField("nodegroup", nodegroup).Info(nodeGroup.scaleUpLock)
		c.logger.WithField("nodegroup", nodegroup).Info("Waiting for scale to finish")
	...
	case maxPercent > float64(nodeGroup.Opts.ScaleUpThresholdPercent):
		...
		if err != nil {
			c.logger.Errorf("Failed to calculate node delta: %v", err)
			return nodesDelta, err
		}
	}

	c.logger.WithField("nodegroup", nodegroup).Debugf("Delta: %v", nodesDelta)
	...
	default:
		c.logger.WithField("nodegroup", nodegroup).Info("No need to scale")
		...
		c.logger.WithField("nodegroup", nodegroup).Infof("Reaper: There were %v empty nodes deleted this round", removed)
	}

	c.logger.WithField("nodegroup", nodegroup).Debugf("DeltaScaled: %v", nodesDeltaResult)
	return nodesDelta, err
}
...
func (c *Controller) RunOnce() {
	...
	for i := 0; i < 2 && err != nil; i++ {
		c.logger.Warnf("cloud provider failed to refresh. trying to re-fetch credentials. tries = %v", i+1)
	...
	for _, nodeGroupOpts := range c.Opts.NodeGroups {
		c.logger.Debugf("**********[START NODEGROUP %v]**********", nodeGroupOpts.Name)
	...
	endTime := time.Now()
	c.logger.Debugf("Scaling took a total of %v", endTime.Sub(startTime))
}
...
func (c *Controller) RunForever(runImmediately bool) {
	if runImmediately {
		c.logger.Debug("**********[AUTOSCALER FIRST LOOP]**********")
		c.RunOnce()
	}
	...
	for {
		select {
		case <-ticker.C:
			c.logger.Debug("**********[AUTOSCALER MAIN LOOP]**********")
			c.RunOnce()
		case <-c.stopChan:
			c.logger.Debugf("Stopping main loop")
			ticker.Stop()
			return
		}
	}
